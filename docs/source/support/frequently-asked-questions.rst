...
By default, the workflows in ``tests-on-pr.yml`` and ``matrix-and-codecov-on-merge-to-main.yml`` will have a section that uploads the code coverage to Codecov, and CI will fail if unsuccessful. If you have a private repository and Codecov cannot be run, CI will always fail, and this is undesirable.

In addition to being able to customize additional commands to be ran in the ``run:`` section (see :ref:`faq-github-actions-extra-cli-commands` above), you can also customize the workflows that you want your repository to run. On top of the ``tests-on-pr.yml`` and ``matrix-and-codecov-on-merge-to-main.yml`` workflows in the ``scikit-package/release-scripts`` repository, we offer ``tests-on-pr-no-codecov.yml`` and ``matrix-no-codecov-on-merge-to-main.yml`` for users in this situation.
